...
#################################################################################################
        ...
        self.x = [datetime.datetime.strptime(date_str, '%Y-%m-%d %H:%M:%S').timestamp() for date_str in self.x]

        points = np.array([self.x, self.y]).T.reshape(-1, 1, 2)
        segments = np.concatenate([points[:-1], points[1:]], axis=1)


        lc = LineCollection(segments, cmap=plt.cm.seismic_r)
        lc.set_array(self.y)
        lc.set_linewidth(self.plt_thickness)
        ...
        # ee1 = time.time()
        # print('latency in first:', ee1 - ss1)
        ...
        # x_labels = [self.x[i] for i in x_indices]  # Get the corresponding formatted x-labels
        ...
        if growth_percentage is not None:
            text = f"Variance: {growth_percentage:.2f}%"
